#' @title Get credible sets for exposure or outcome for the Z-score implementation
#'
...
#'
#' @param R The list of length M of correlation matrices
#'
#' @return A list of lists containing the credible sets for each region
#'
#' @export

get_cs_rss <- function(V, alpha, R){
  M <- length(alpha)
  ...
  for(m in 1:M){
    ...
    if (sum(include_idx) == 0){
      ...
      for (j in 1:length(cs)) {
        purity <- rbind(purity, matrix(susieR:::get_purity(cs[[j]], X = NULL, Xcorr = R[[m]], squared = FALSE, n = 100, use_rfast), 1, 3)) #double check the muffled_cov2cor?
      }
